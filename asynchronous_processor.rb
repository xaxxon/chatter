...
module AsynchronousProcessorBase
  ...
  def run(time)
    ...
    if @previous_execution_time.nil? or time >= @previous_execution_time + self.period_seconds
      if @previous_execution_time.nil?
        @previous_execution_time = time
      else
        @previous_execution_time += self.period_seconds # not the ACTUAL time called, but the time it SHOULD have been called otherwise we'll always lag
      end
      @complete = self._run
    end
  end
  ...
  def time_til_next_run
    @previous_execution_time - @most_recent_time_called + self.period_seconds
  end
